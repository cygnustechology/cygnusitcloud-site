...
const BackupExport = () => {
  ...
  const exportConfig = async () => {
    setExporting("config");
    try {
      const [connections, apps, databases, users] = await Promise.all([getConnections(), getApps(), getDatabases(), getAllUsers()]);
      const data = { exportedAt: new Date().toISOString(), platform: "Cygnus Cloud", version: "1.0.0", connections: connections.map(c => ({ ...c, agent_secret: "***REDACTED***" })), apps, databases, tenants: users.map(t => ({ ...t, password_hash: "***REDACTED***" })) };
  ...
  const importConfig = () => {
    ...
    input.onchange = (e) => {
      ...
      reader.onload = (ev) => { try { const data = JSON.parse(ev.target?.result as string); if (data.platform !== "Cygnus Cloud") { toast.error("Invalid backup file"); return; } toast.success("Config imported successfully."); } catch { toast.error("Invalid JSON file"); } };
